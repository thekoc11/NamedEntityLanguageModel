...
import model_ori_with_type as model
...
parser.add_argument('--save', type=str,  default='composite'+'.pt',
...
val_data2 = batchify(corpus.valid_type, eval_batch_size, args)
test_data2 = batchify(corpus.test_type, test_batch_size, args)
...
print ('vocab with all type and ori: ', ntokens)
...
def evaluate(data_source, data_source2, batch_size=10):
...
try:
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if 't0' in optimizer.param_groups[0]:
            ...
            val_loss2 = evaluate(val_data, val_data2)
            print('-' * 89)
            ...
            val_loss = evaluate(val_data, val_data2, eval_batch_size)
            print('-' * 89)
            print('| end of epoch {:3d} | time: {:5.2f}s | valid loss {:5.2f} | '
                    'valid ppl {:8.2f}'.format(epoch, (time.time() - epoch_start_time),
...
# Run on test data.
test_loss = evaluate(test_data, test_data2, test_batch_size)
print('=' * 89)
