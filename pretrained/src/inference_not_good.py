...
def get_word_map():
    id_map ={}
    i = 0
    for i in range(len(corpus2.dictionary.idx2word)):
        word = corpus2.dictionary.idx2word[i]
        id_map.update({i:corpus.dictionary.word2idx[word]})
...
def convert_type_score_to_var_score(output_flat2, output_flat):
    # print output_flat2.size() # batch x bptt x len(dictionary) now batch is 1 so size() = bptt x len(dictionary) 
    assert ntokens2== output_flat2.size()[1]
    # print(output_flat.type(), output_flat2.type())
    output_flat.data = output_flat.data.fill_(0)
    # word_map = get_word_map()
    for i in range(ntokens2):
        # print i, word_map[i]
        output_flat[:,word_map[i]] = output_flat2[:, i].data
    return output_flat
...
def evaluate_both(data_source, data_source_type, data_source2, batch_size=10):
    ...
    for batch,i in enumerate(range(0, data_source.size(0) - 1, args.bptt)):
        ...
        # output_flat_cb= convert_type_score_to_var_score(output_flat2.clone(), output_flat.clone())
    	for idxx in range(numwords):
    		for pos in candidates_ids: #for all candidates
                ...
                # output_flat_cb[idxx][pos] = new_prob1
