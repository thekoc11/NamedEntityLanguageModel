    ...
    for batch,i in enumerate(range(0, data_source.size(0) - 1, args.bptt)):
        ...
        for idxx in range(numwords):
            for pos in candidates_ids: #for all candidates
                ...
                # else: 
                #     sum_ = 0
                #     prob = m(output_flat2.data)
                #     for t in ['fruits', 'dairy', 'drinks', 'sides', 'grains', 'proteins', 'seasonings', 'vegetables']:
                #         sum_ += prob[idxx][corpus2.dictionary.word2idx[t]]
                #     sums.append(sum_)
                    
                # print ('word: ', corpus.dictionary.idx2word[pos], sum_, 'not prob:', sum(prob[idxx]) - sum_)
                output_flat_cb.data[idxx][pos] = new_prob1
        # print ('unique: ', set(sums))
        # exit()
