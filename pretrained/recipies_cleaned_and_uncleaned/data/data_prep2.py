...
def text_preprocess(text, ing = False, title = False):
    # remove all non-alphabet characters
    if title==False:modified_text = re.sub(r'[^a-z.A-Z]', ' ', text)
    ...
    for t in allowed_tokens:
        ...
        if len(tokens)>0 and tokens[-1] in stopWords and t==".": continue
        if len(tokens)>0 and tokens[-1]=='.' and t==".": continue
