...
class RNNModel(nn.Module):
    ...
    def __init__(self, rnn_type, ntoken, ninp, nhid, nlayers, dropout=0.5, dropouth=0.5, dropouti=0.5, dropoute=0.1, wdrop=0, tie_weights=False):
        ...
        if rnn_type == 'LSTM':
            self.rnns = [torch.nn.LSTM(2*ninp if l == 0 else nhid, nhid if l != nlayers - 1 else (ninp if tie_weights else nhid), 1, dropout=0) for l in range(nlayers)]
            if wdrop:
    ...
    def forward(self, input, input2, hidden, return_h=False):
        ...
        emb2 = embedded_dropout(self.encoder, input2, dropout=self.dropoute if self.training else 0)
        #emb = self.idrop(emb)
